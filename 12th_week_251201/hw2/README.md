...
### Vue.js 사용 이유
- 바닐라 JS는 과제 1번에서 했었고, 백엔드 처리 등의 코드 작성에 시간이 많이 걸렸습니다.
- 또한 FastAPI에서 프론트엔드를 StaticFile로 서빙하는 것에 대해서는 별다른 차이가 없다는 점도 Vue.js로 설계한 이유입니다.
- 현재 프론트엔드쪽은 React, Vue, Svelte의 3강 체제로 알고 있습니다. 이들은 바닐라에서 복잡하게 구현해야 하는 상태 관리나 동적렌더링을 위한 편의 기능이 많다는 것이 장점이라고 생각합니다.
